...
namespace LAMMPS_NS {
struct ACECKimpl {
  ACECKimpl() : basis_set(nullptr), ace(nullptr) {}
  ~ACECKimpl()
  {
    delete basis_set;
    delete ace;
...
//ComputePACEKokkos<DeviceType>::ComputePACEKokkos(LAMMPS *lmp, int narg, char **arg) : ComputePACE(lmp, narg, arg)
ComputePACEKokkos<DeviceType>::ComputePACEKokkos(LAMMPS *lmp, int narg, char **arg) : Compute(lmp, narg, arg)
//ComputePACEKokkos<DeviceType>::ComputePACEKokkos(LAMMPS *lmp, int narg, char **arg) : ComputePACE(lmp)
//ComputePACEKokkos<DeviceType>::ComputePACEKokkos(lmp, narg, arg) : ComputePACE(lmp, narg, arg)
//ComputePACEKokkos<DeviceType>::ComputePACEKokkos(*lmp, narg, **arg) : ComputePACE(lmp)
{
  //respa_enable = 0;

  kokkosable = 1;
  atomKK = (AtomKokkos *) atom;
  aceckimpl = new ACECKimpl;
...
template<class DeviceType>
void ComputePACEKokkos<DeviceType>::grow(int natom, int maxneigh)
{
  //auto basis_set = ComputePACE::acecimpl->basis_set;
  auto basis_set = aceckimpl->basis_set;
...
/*
template<class DeviceType>
double ComputePACEKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = ComputePACE::init_one(i,j);
...
/*
template<class DeviceType>
void ComputePACEKokkos<DeviceType>::coeff(int narg, char **arg)
{
  ComputePACE::coeff(narg,arg);
...
template<class DeviceType>
void ComputePACEKokkos<DeviceType>::allocate()
{
  ComputePACEKokkos::allocate();
...
void ComputePACEKokkos<DeviceType>::compute_array()
{
  if (host_flag) {
    ...
    ComputePACEKokkos::compute_array();
 ...
 /*
  ...
  if (!force->newton_pair)
    error->all(FLERR,"ComputePACEKokkos requires 'newton on'");
  ...
  while (chunk_offset < inum) { // chunk up loop to prevent running out of memory
    ...
    //Kokkos::deep_copy(d_d_min, ComputePACE::acecimpl->basis_set->cutoffmax);
    Kokkos::deep_copy(d_d_min, aceckimpl->basis_set->cutoffmax);
    ...
    //Neigh
    {
      ...
      typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeNeigh> policy_neigh(chunk_size,team_size,vector_length);
    ...
    //ComputeRadial
    {
      int vector_length = vector_length_default;
      int team_size = team_size_default;
      check_team_size_for<TagComputePACEComputeRadial>(((chunk_size+team_size-1)/team_size)*maxneigh,team_size,vector_length);
      typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeRadial> policy_radial(((chunk_size+team_size-1)/team_size)*maxneigh,team_size,vector_length);
    ...
    //ComputeAi
    {
      int vector_length = vector_length_default;
      int team_size = team_size_default;
      check_team_size_for<TagComputePACEComputeAi>(((chunk_size+team_size-1)/team_size)*maxneigh,team_size,vector_length);
      typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeAi> policy_ai(((chunk_size+team_size-1)/team_size)*maxneigh,team_size,vector_length);
      Kokkos::parallel_for("ComputeAi",policy_ai,*this);
    }

    //ConjugateAi
    {
      typename Kokkos::RangePolicy<DeviceType,TagComputePACEConjugateAi> policy_conj_ai(0,chunk_size);
      Kokkos::parallel_for("ConjugateAi",policy_conj_ai,*this);
    }

    //ComputeRho
    {
      typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeRho> policy_rho(0,chunk_size*idx_ms_combs_max);
      Kokkos::parallel_for("ComputeRho",policy_rho,*this);
    }

    //ComputeFS
    {
      typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeFS> policy_fs(0,chunk_size);
      Kokkos::parallel_for("ComputeFS",policy_fs,*this);
    }

    //ComputeWeights
    {
      typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeWeights> policy_weights(0,chunk_size * idx_ms_combs_max);
    ...
    //ComputeDerivative
    {
      int vector_length = vector_length_default;
      int team_size = team_size_default;
      check_team_size_for<TagComputePACEComputeDerivative>(((chunk_size+team_size-1)/team_size)*maxneigh,team_size,vector_length);
      typename Kokkos::TeamPolicy<DeviceType,TagComputePACEComputeDerivative> policy_derivative(((chunk_size+team_size-1)/team_size)*maxneigh,team_size,vector_length);
    ...
    /*
    {
      if (evflag) {
        if (neighflag == HALF) {
          typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeForce<HALF,1> > policy_force(0,chunk_size);
          Kokkos::parallel_reduce(policy_force, *this, ev_tmp);
        } else if (neighflag == HALFTHREAD) {
          typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeForce<HALFTHREAD,1> > policy_force(0,chunk_size);
          Kokkos::parallel_reduce("ComputeForce",policy_force, *this, ev_tmp);
        }
      } else {
        if (neighflag == HALF) {
          typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeForce<HALF,0> > policy_force(0,chunk_size);
          Kokkos::parallel_for(policy_force, *this);
        } else if (neighflag == HALFTHREAD) {
          typename Kokkos::RangePolicy<DeviceType,TagComputePACEComputeForce<HALFTHREAD,0> > policy_force(0,chunk_size);
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeNeigh,const typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeNeigh>::member_type& team) const
{
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeRadial, const typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeRadial>::member_type& team) const
{
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeAi, const typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeAi>::member_type& team) const
{
  // Extract the atom number
  int ii = team.team_rank() + team.team_size() * (team.league_rank() %
           ((chunk_size+team.team_size()-1)/team.team_size()));
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEConjugateAi, const int& ii) const
{
  for (int mu_j = 0; mu_j < nelements; mu_j++) {
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeRho, const int& iter) const
{
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeFS, const int& ii) const
{
  const int i = d_ilist[ii + chunk_offset];
  const int mu_i = d_map(type(i));
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeWeights, const int& iter) const
{
  const int idx_ms_combs = iter / chunk_size;
  const int ii = iter % chunk_size;
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeDerivative, const typename Kokkos::TeamPolicy<DeviceType, TagComputePACEComputeDerivative>::member_type& team) const
{
  // Extract the atom number
  int ii = team.team_rank() + team.team_size() * (team.league_rank() %
           ((chunk_size+team.team_size()-1)/team.team_size()));
...
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeForce<NEIGHFLAG,EVFLAG>, const int& ii, EV_FLOAT& ev) const
{
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::operator() (TagComputePACEComputeForce<NEIGHFLAG,EVFLAG>,const int& ii) const {
  EV_FLOAT ev;
  this->template operator()<NEIGHFLAG,EVFLAG>(TagComputePACEComputeForce<NEIGHFLAG,EVFLAG>(), ii, ev);
}
...
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::v_tally_xyz(EV_FLOAT &ev, const int &i, const int &j,
...
template<class DeviceType>
void ComputePACEKokkos<DeviceType>::pre_compute_harmonics(int lmax)
{
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::cutoff_func_poly(const double r, const double r_in, const double delta_in, double &fc, double &dfc) const
{
  if (r <= r_in-delta_in) {
    fc = 1;
    dfc = 0;
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::Fexp(const double x, const double m, double &F, double &DF) const
{
  const double w = 1.e6;
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::FexpShiftedScaled(const double rho, const double mexp, double &F, double &DF) const
{
  const double eps = 1e-10;
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::inner_cutoff(const double rho_core, const double rho_cut, const double drho_cut,
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::FS_values_and_derivatives(const int ii, double &evdwl, const int mu_i) const
{
  double F, DF = 0;
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::evaluate_splines(const int ii, const int jj, double r,
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
void ComputePACEKokkos<DeviceType>::SplineInterpolatorKokkos::operator=(const SplineInterpolator &spline) {
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void ComputePACEKokkos<DeviceType>::SplineInterpolatorKokkos::calcSplines(const int ii, const int jj, const double r, const t_ace_3d &d_values, const t_ace_3d &d_derivatives) const
{
...
void ComputePACEKokkos<DeviceType>::check_team_size_for(int inum, int &team_size, int vector_length) {
  int team_size_max;
...
template<class DeviceType>
template<class TagStyle>
void ComputePACEKokkos<DeviceType>::check_team_size_reduce(int inum, int &team_size, int vector_length) {
  int team_size_max;
...
template<class DeviceType>
double ComputePACEKokkos<DeviceType>::memory_usage()
{
  double bytes = 0;
