...
ComputePACEGridKokkos<DeviceType, real_type, vector_length>::ComputePACEGridKokkos(LAMMPS *lmp, int narg, char **arg) : ComputePACEGrid(lmp, narg, arg)
{
  kokkosable = 1;
  ...
  k_cutsq = tdual_fparams("ComputePACEGridKokkos::cutsq",atom->ntypes+1,atom->ntypes+1);
  ...
  MemKK::realloc_kokkos(d_radelem,"ComputePACEGridKokkos::radelem",nelements);
  MemKK::realloc_kokkos(d_wjelem,"ComputePACEGridKokkos:wjelem",nelements);
  MemKK::realloc_kokkos(d_sinnerelem,"ComputePACEGridKokkos:sinnerelem",nelements);
  MemKK::realloc_kokkos(d_dinnerelem,"ComputePACEGridKokkos:dinnerelem",nelements);
  // test
  MemKK::realloc_kokkos(d_test, "ComputePACEGridKokkos::test", nelements);

  int n = atom->ntypes;
  MemKK::realloc_kokkos(d_map,"ComputePACEGridKokkos::map",n+1);
  ...
  snaKK = PACEKokkos<DeviceType, real_type, vector_length>(*this);
  snaKK.grow_rij(0,0);
  snaKK.init();
}
...
template<class DeviceType, typename real_type, int vector_length>
ComputePACEGridKokkos<DeviceType, real_type, vector_length>::~ComputePACEGridKokkos()
{
  if (copymode) return;
...
void ComputePACEGridKokkos<DeviceType>::grow(int natom, int maxneigh)
{
  ...
  if ((int)A.extent(0) < natom) {
    ...
    MemKK::realloc_kokkos(A_list, "pace/grid:A_list", natom, idx_ms_combs_max, basis_set->rankmax);
    //size is +1 of max to avoid out-of-boundary array access in double-triangular scheme
    MemKK::realloc_kokkos(A_forward_prod, "pace/grid:A_forward_prod", natom, idx_ms_combs_max, basis_set->rankmax + 1);

    MemKK::realloc_kokkos(e_atom, "pace/grid:e_atom", natom);
    MemKK::realloc_kokkos(rhos, "pace/grid:rhos", natom, basis_set->ndensitymax + 1); // +1 density for core repulsion
    MemKK::realloc_kokkos(dF_drho, "pace/grid:dF_drho", natom, basis_set->ndensitymax + 1); // +1 density for core repulsion

    MemKK::realloc_kokkos(weights, "pace/grid:weights", natom, nelements, idx_sph_max, nradmax + 1);
    MemKK::realloc_kokkos(weights_rank1, "pace/grid:weights_rank1", natom, nelements, nradbase);

    // hard-core repulsion
    MemKK::realloc_kokkos(rho_core, "pace/grid:rho_core", natom);
    MemKK::realloc_kokkos(dF_drho_core, "pace/grid:dF_drho_core", natom);
    MemKK::realloc_kokkos(dF_dfcut, "pace/grid:dF_dfcut", natom);
    MemKK::realloc_kokkos(d_d_min, "pace/grid:r_min_pair", natom);
    MemKK::realloc_kokkos(d_jj_min, "pace/grid:j_min_pair", natom);
    MemKK::realloc_kokkos(d_corerep, "pace/grid:corerep", natom); // per-atom corerep

    MemKK::realloc_kokkos(dB_flatten, "pace/grid:dB_flatten", natom, idx_ms_combs_max, basis_set->rankmax);
  }

  if (((int)fr.extent(0) < natom) || ((int)fr.extent(1) < maxneigh)) {

    // radial functions
    MemKK::realloc_kokkos(fr, "pace/grid:fr", natom, maxneigh, lmax + 1, nradmax);
    MemKK::realloc_kokkos(dfr, "pace/grid:dfr", natom, maxneigh, lmax + 1, nradmax);
    MemKK::realloc_kokkos(gr, "pace/grid:gr", natom, maxneigh, nradbase);
    MemKK::realloc_kokkos(dgr, "pace/grid:dgr", natom, maxneigh, nradbase);
    const int max_num_functions = MAX(nradbase, nradmax*(lmax + 1));
    MemKK::realloc_kokkos(d_values, "pace/grid:d_values", natom, maxneigh, max_num_functions);
    MemKK::realloc_kokkos(d_derivatives, "pace/grid:d_derivatives", natom, maxneigh, max_num_functions);

    // hard-core repulsion
    MemKK::realloc_kokkos(cr, "pace/grid:cr", natom, maxneigh);
    MemKK::realloc_kokkos(dcr, "pace/grid:dcr", natom, maxneigh);

    // short neigh list
    MemKK::realloc_kokkos(d_ncount, "pace/grid:ncount", natom);
    MemKK::realloc_kokkos(d_mu, "pace/grid:mu", natom, maxneigh);
    MemKK::realloc_kokkos(d_rhats, "pace/grid:rhats", natom, maxneigh);
    MemKK::realloc_kokkos(d_rnorms, "pace/grid:rnorms", natom, maxneigh);
    MemKK::realloc_kokkos(d_nearest, "pace/grid:nearest", natom, maxneigh);

    MemKK::realloc_kokkos(f_ij, "pace/grid:f_ij", natom, maxneigh);
  }
}
...
template<class DeviceType, typename real_type, int vector_length>
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::setup()
{
...
template<class DeviceType, typename real_type, int vector_length>
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::compute_array()
{
  if (host_flag) {
    ComputePACEGrid::compute_array();
    return;
  }
  ...
  MemKK::realloc_kokkos(d_ninside,"ComputePACEGridKokkos:ninside",total_range);
  ...
  while (chunk_offset < total_range) { // chunk up loop to prevent running out of memory
    ...
    //ComputeNeigh
    {
      ...
      SnapAoSoATeamPolicy<DeviceType, team_size_compute_neigh, TagCPACEGridComputeNeigh>
    ...
    //ComputeCayleyKlein
    {
      ...
      Snap3DRangePolicy<DeviceType, tile_size_compute_ck, TagCPACEGridComputeCayleyKlein>
    ...
    //PreUi
    {
      auto policy_pre_ui = snap_get_policy<DeviceType, tile_size_pre_ui, TagCPACEGridPreUi>(chunk_size_div, twojmax + 1);
    ...
    // ComputeUi w/ vector parallelism, shared memory, direct atomicAdd into ulisttot
    {
      ...
      if (chunk_size < parallel_thresh)
      {
        ...
        SnapAoSoATeamPolicy<DeviceType, team_size_compute_ui, TagCPACEGridComputeUiSmall>
        ...
        SnapAoSoATeamPolicy<DeviceType, team_size_compute_ui, TagCPACEGridComputeUiLarge>
          policy_ui(n_teams_div, team_size_compute_ui, vector_length);
        policy_ui = policy_ui.set_scratch_size(0, Kokkos::PerTeam(scratch_size));
    ...
    //TransformUi: un-"fold" ulisttot, zero ylist
    {
      ...
      // Zero out ylist
      auto policy_transform_ui = snap_get_policy<DeviceType, tile_size_transform_ui, TagCPACEGridTransformUi>(chunk_size_div, snaKK.idxu_max);
    ...
    //ComputeZi and Bi
    if (nelements > 1) {
      auto policy_compute_zi = snap_get_policy<DeviceType, tile_size_compute_zi, TagCPACEGridComputeZi<true>, min_blocks_compute_zi>(chunk_size_div, snaKK.idxz_max);
      Kokkos::parallel_for("ComputeZiChemsnap", policy_compute_zi, *this);

      auto policy_compute_bi = snap_get_policy<DeviceType, tile_size_compute_bi, TagCPACEGridComputeBi<true>>(chunk_size_div, snaKK.idxb_max);
      Kokkos::parallel_for("ComputeBiChemsnap", policy_compute_bi, *this);
    } else {
      auto policy_compute_zi = snap_get_policy<DeviceType, tile_size_compute_zi, TagCPACEGridComputeZi<false>, min_blocks_compute_zi>(chunk_size_div, snaKK.idxz_max);
      Kokkos::parallel_for("ComputeZi", policy_compute_zi, *this);

      auto policy_compute_bi = snap_get_policy<DeviceType, tile_size_compute_bi, TagCPACEGridComputeBi<false>>(chunk_size_div, snaKK.idxb_max);
      Kokkos::parallel_for("ComputeBi", policy_compute_bi, *this);
    }

    // Fill the grid array with bispectrum values
    {
      typename Kokkos::RangePolicy<DeviceType,TagCPACEGridLocalFill> policy_fill(0,chunk_size);
...
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeNeigh,const typename Kokkos::TeamPolicy<DeviceType,TagCPACEGridComputeNeigh>::member_type& team) const {
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeCayleyKlein,const int iatom_mod, const int jnbor, const int iatom_div) const {
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridPreUi, const int& iatom_mod, const int& j, const int& iatom_div) const {
  const int iatom = iatom_mod + iatom_div * vector_length;
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridPreUi, const int& iatom, const int& j) const {
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridPreUi, const int& iatom) const {
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeUiSmall,const typename Kokkos::TeamPolicy<DeviceType,TagCPACEGridComputeUiSmall>::member_type& team) const {
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeUiLarge,const typename Kokkos::TeamPolicy<DeviceType,TagCPACEGridComputeUiLarge>::member_type& team) const {
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridTransformUi, const int& iatom_mod, const int& idxu, const int& iatom_div) const {
  const int iatom = iatom_mod + iatom_div * vector_length;
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridTransformUi, const int& iatom, const int& idxu) const {
  if (iatom >= chunk_size) return;
  snaKK.transform_ui(iatom, idxu);
}
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridTransformUi, const int& iatom) const {
  if (iatom >= chunk_size) return;
...
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeZi<chemsnap>, const int& iatom_mod, const int& jjz, const int& iatom_div) const {
  const int iatom = iatom_mod + iatom_div * vector_length;
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
template <bool chemsnap> KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeZi<chemsnap>, const int& iatom, const int& jjz) const {
  if (iatom >= chunk_size) return;
  snaKK.template compute_zi<chemsnap>(iatom, jjz);
}
...
template<class DeviceType, typename real_type, int vector_length>
template <bool chemsnap> KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeZi<chemsnap>, const int& iatom) const {
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
template <bool chemsnap> KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeBi<chemsnap>, const int& iatom_mod, const int& jjb, const int& iatom_div) const {
  const int iatom = iatom_mod + iatom_div * vector_length;
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
template <bool chemsnap> KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeBi<chemsnap>, const int& iatom, const int& jjb) const {
  if (iatom >= chunk_size) return;
  snaKK.template compute_bi<chemsnap>(iatom, jjb);
}
...
template<class DeviceType, typename real_type, int vector_length>
template <bool chemsnap> KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridComputeBi<chemsnap>, const int& iatom) const {
  if (iatom >= chunk_size) return;
...
template<class DeviceType, typename real_type, int vector_length>
KOKKOS_INLINE_FUNCTION
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::operator() (TagCPACEGridLocalFill, const int& ii) const {
...
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::check_team_size_for(int inum, int &team_size) {
  int team_size_max;
...
template<class DeviceType, typename real_type, int vector_length>
template<class TagStyle>
void ComputePACEGridKokkos<DeviceType, real_type, vector_length>::check_team_size_reduce(int inum, int &team_size) {
  int team_size_max;
...
int ComputePACEGridKokkos<DeviceType, real_type, vector_length>::scratch_size_helper(int values_per_team) {
...
template<class DeviceType>
ComputePACEGridKokkosDevice<DeviceType>::ComputePACEGridKokkosDevice(class LAMMPS *lmp, int narg, char **arg)
   : ComputePACEGridKokkos<DeviceType, SNAP_KOKKOS_REAL, SNAP_KOKKOS_DEVICE_VECLEN>(lmp, narg, arg) { ; }

template<class DeviceType>
void ComputePACEGridKokkosDevice<DeviceType>::compute_array()
{
  Base::compute_array();
}
...
template<class DeviceType>
ComputePACEGridKokkosHost<DeviceType>::ComputePACEGridKokkosHost(class LAMMPS *lmp, int narg, char **arg)
   : ComputePACEGridKokkos<DeviceType, SNAP_KOKKOS_REAL, SNAP_KOKKOS_HOST_VECLEN>(lmp, narg, arg) { ; }

template<class DeviceType>
void ComputePACEGridKokkosHost<DeviceType>::compute_array()
{
  Base::compute_array();
}
#endif
